...
const char *
condition_field(json_t *current_condition, const char *key) {
  json_t *field_val =
    json_object_get(json_array_get(current_condition, 0), key);
...
main(int argc, char *argv[]) {
  ...
  if(root) {
	...
	condition_field(current_condition, "weatherDesc");
      const char *wind_speed =
	condition_field(current_condition, "windspeedKmph");
      const char *wind_degree =
	condition_field(current_condition, "winddirDegree");

      printf("Temprature: %s\nWeather description: %s\nWind speed: %s\nWind "
	     "degree: %s",
	     temp_c, weather_desc, wind_speed,
	     convert_to_angle(atoi(wind_degree)));
    }
