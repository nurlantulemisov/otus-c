...
bool
read_eocdr(FILE *f, struct eocdr *r, const size_t sz) {
  uint32_t signature;
...
bool
read_cfh(FILE *f, struct cfh *c, size_t f_sz, size_t offset) {
  uint32_t signature;
...
bool
read_lfh(FILE *f, struct lfh *l, size_t f_sz, size_t offset) {
  uint32_t signature;
...
bool
is_zip(const char *path) {
  if(path == NULL) {
    // return 1;
    path =
      "/home/nurlantulemisov/myprojects/otus-clang/hw1/assets/zipjpeg2.jpg";
  }
